...
class SOSTriggerManager(private val context: Context) {
    ...
    private fun initiateSOS() {
        ...
        val userManager = com.rohit.sosafe.data.UserManager(context)
        if (!userManager.hasContactsSync()) {
            Log.w("SOSTriggerManager", "SOS_ABORTED: No guardians linked.")
            vibrateWarning()
            android.widget.Toast.makeText(context, "Cannot trigger SOS: No emergency guardians linked", android.widget.Toast.LENGTH_LONG).show()
            return
        }
