...
class CloudinaryUploader {
    ...
    fun uploadVideo(
        ...
        val url = "https://api.cloudinary.com/v1_1/$CLOUD_NAME/video/upload"
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("CloudinaryUploader", "Video Upload failed for ${file.name}", e)
                onFailure(e)
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                response.use {
                    if (!response.isSuccessful) {
                        val errorMsg = response.body?.string() ?: "Unknown error"
                        Log.e("CloudinaryUploader", "Video Upload unsuccessful: $errorMsg")
                        onFailure(IOException("Unexpected code $response"))
                        return
                    }
                    ...
                    if (secureUrl.isNotEmpty()) {
                        Log.d("CloudinaryUploader", "Video Upload successful: $secureUrl")
                        onSuccess(secureUrl)
                    } else {
                        onFailure(IOException("Secure URL not found in response"))
                    }
